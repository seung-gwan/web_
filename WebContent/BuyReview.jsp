...
<html lang="ko">
  ...
  <body class="blue-page">
    ...
    <main>
      ...
      <section class="support-section" aria-labelledby="buy-review-title">
        <nav class="support-tabs" aria-label="고객센터 메뉴">
          <a href="Notice.do">공지사항</a>
          <a href="BuyReview.do" aria-current="page">구매후기</a>
          <a href="SellReview.do">판매후기</a>
        </nav>

        <div class="support-heading">
          <div>
            <span class="blue-eyebrow">BUY STORIES</span>
            <h2 id="buy-review-title">구매후기</h2>
          </div>
          <p>차량을 구매한 고객의 생생한 경험을 확인해 보세요.</p>
        </div>

        <div class="notice-card review-card">
          <div class="review-toolbar">
            <form class="review-search" id="reviewSearch" role="search">
              <label class="hidden" for="review-search-box">구매후기 검색</label>
              <input type="search" id="review-search-box" placeholder="제목, 작성자 또는 차량명 검색" autocomplete="off">
              <button type="submit">검색</button>
            </form>
            <a class="review-write-button" href="BuyReview_write.jsp">후기 작성하기</a>
          </div>

          <div class="notice-table-wrap">
            <table class="notice-table review-table">
              <caption class="hidden">구매후기 목록</caption>
              <thead>
                <tr>
                  <th scope="col">번호</th>
                  <th scope="col">제목</th>
              ...
              <tbody>
                <c:forEach items="${BuyReview}" var="dto">
                  <tr class="review-row">
                    <td>${dto.bRNum}</td>
                    <td class="notice-table__title"><a href="BuyReview_view.do?bRNum=${dto.bRNum}">${dto.bRTitle}</a></td>
                    <td>${dto.bRId}</td>
                    <td><span class="review-car-badge">${dto.bRCar}</span></td>
                    <td><fmt:formatDate value="${dto.bRDate}" pattern="yyyy.MM.dd"/></td>
                    <td>${dto.bRHit}</td>
                  </tr>
                </c:forEach>
                <c:if test="${empty BuyReview}">
                  <tr class="notice-empty-row"><td colspan="6">등록된 구매후기가 없습니다.</td></tr>
                </c:if>
              </tbody>
            </table>
          </div>
